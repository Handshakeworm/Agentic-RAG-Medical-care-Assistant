...
# - _TABLE_SUMMARY_SYSTEM: text LLM 看 html,无视觉负担
...
_TABLE_SUMMARY_SYSTEM = """你是医学教科书写作助手。给定一张医学表格的 html、所属章节路径、caption,你需要同时产出 4 个字段(详见下方"4 字段说明"),作为本表 *_summary chunk 入库及多向量检索的全套元数据。
...
- **footnote(表注)必须用于解读**:输入若含 footnote 段,通常包含缩写释义(如"福:福善美;罗:罗盖全")、星号/上标定义(如"*为血管内溶血"、"`#`:每隔 15 分钟增加剂量")、关键标注解释(如"CAS≥3 分即为 GO 活动")。这些信息 html cell 里只有缩写或符号,完整含义全靠 footnote 提供,**必须用 footnote 反向解读 html 后再写 medical_statement**。但**不要把"资料来源""引自...""(参考 X et al. 2016)"等纯出处信息写进 medical_statement / summary**——这些只是 mineru 顺带带进来的引用尾,不是知识本体
...
5. **数值/单位/范围必须原样保留**(医学表的核心价值就在精度):不要四舍五入(`0.05` 不能写成 `0.1`)、不要简化单位(`mg/(kg·d)` 不能写成 `mg/kg`,`μmol/L` 不能写成 `mol/L`)、不要省略小数点、不要换算单位、不要把"3.5-5.5 mmol/L"简化成"约 5 mmol/L"。原表给什么数你写什么数

# 大表的组织策略

- **列对比表**(如鉴别诊断 A vs B vs C):提炼差异维度,用对比句式陈述
- **数值参考表**(如肝功能正常值):按指标分组陈述,**关键阈值原样写出 + 临床意义**
- **用药剂量表**:按疾病/分期组织,**剂量数值/单位/给药频次原样写出 + 适应证**""" + _SHARED_4FIELD_TAIL + """
...
- **footnote(图注)必须用于解读**:输入若含 footnote 段,通常包含图例语义(如"实线为正性效应,虚线为负性效应")、单位/数值定义(如"图中数字为氧分压 mmHg")、缩写释义(如"CTD:结缔组织疾病")、标注解释(如 `*` 号含义)。这些信息**截图中肉眼看不到完整含义**,必须用 footnote 反向理解图后再写 medical_statement。但**不要把"参考文献""资料来源""引自本章文献 N"等纯出处信息写进 medical_statement / summary**——这些只是 mineru 顺带带进来的引用尾,不是知识本体
...
_TABLE_SUMMARY_USER_TEMPLATE = """# 所属章节
{heading_path}
...
请基于以上 html 表格 + footnote,产出 4 字段 JSON(medical_statement / title / summary / hypothetical_questions)。"""
...
请按"认图 → 抽视觉要素 → 转写医学陈述"三步内化在脑内推理,基于下方截图 + footnote 产出 4 字段 JSON(medical_statement / title / summary / hypothetical_questions)。"""


def build_table_summary_messages(
    heading_path: str,
    caption: str,
    footnote: str,
    mineru_sub_type: str,
    ...
    """table 专用(纯文本 LLM 看 html → 单步产 4 字段)。

    被 scripts/table_enrichment_generation.py 使用。
    chart/figure 用 build_figure_summary_multimodal_messages(图像多模态)。

    footnote 含表注:缩写释义、星号定义、关键标注——必传,不传 LLM 看 html 里
    "福/罗/固"或"CTD/PM-DM"这种缩写完全没法解读。
    """
    return [
        ("system", _TABLE_SUMMARY_SYSTEM),
        ("user", _TABLE_SUMMARY_USER_TEMPLATE.format(
            heading_path=heading_path,
            caption=caption or "(无 caption)",
            footnote=footnote or "(无 footnote)",
