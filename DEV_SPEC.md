...
│   └── build_icd10.py                 # 术语库构建脚本 — ICD-10 灌库（→ terms_collection）
│
...
- **输入**：RRF 融合 + 多向量去重后的 Top-M 候选（见 3.2.2），每条候选为 [query, original_content] 对
...
chunks (
  ...
  embedding_status      VARCHAR(20) NOT NULL DEFAULT 'pending',
                                          ...
                                          -- skip：父块专用，永不向量化，补偿任务跳过此类记录（见 3.1.2 父子索引）
  created_at            TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at            TIMESTAMPTZ NOT NULL DEFAULT now()
)
```
...
raw_documents (
  ...
  content_list     JSONB NOT NULL,        -- target_document_content_list.json
                                          -- 含每个内容块的类型（文本/表格/图像）、文本、页码、坐标 bbox
                                          -- 主要用途：chunking 阶段识别表格块做双粒度处理（见 3.1.1）；bbox 仅作版面追溯/解析调试参考，当前摄取管道不对图像类内容做内容理解
  middle_data      JSONB,                 -- target_document_middle.json
                                          -- 含版面分析结构，排查解析异常时使用
  model_data       JSONB,                 -- target_document_model.json（可选）
...
| 字段 | 来源 | 主要用途 |
|------|------|---------|
| `markdown_content` | `target_document.md` | Chunking pipeline 的直接输入（见 3.1.2） |
| `content_list` | `content_list.json` | 内容块类型/文本/页码/bbox；chunking 阶段用于识别表格块做双粒度处理（见 3.1.1）；图像类内容仅留 bbox 供版面追溯，不做内容理解 |
| `middle_data` | `middle.json` | 版面结构存档，供解析异常排查使用 |
| `model_data` | `model.json` | 模型推理细节，默认不写入，按需存储 |
...
| 编号 | 任务 | 产出文件 | 验收标准 |
|------|------|---------|---------|
| C1 | MinerU 产物加载器 | `src/rag/ingestion/mineru_loader.py` | 读取 `mineru_output/` 下的 `.md` + `content_list.json`；写入 PostgreSQL `raw_documents` 表；单元测试 |
| C2 | Chunking（父子分块 + 表格双粒度） | `src/rag/ingestion/chunking.py` | **非表格内容**：先按 heading 边界切出父块（写入 chunks 表，`embedding_status='skip'`，不向量化），再对父块内容执行 `RecursiveCharacterTextSplitter` 切子块并记录 `parent_chunk_id`；输出携带 heading_path、relative_chunk_index。**表格内容**（识别 MinerU `content_list` 中的 table 类型块）：产出两类 chunk——① 整表摘要 chunk（一条概括性描述，回答"这张表讲什么"）② 逐行 chunk（每行转自然语言句子 + 元数据 `{table, row}`）；表格 chunk 的 `parent_chunk_id` 指向所在 heading 节的父块，便于父块扩展时拿到上下文；单元测试覆盖：父块生成、parent_chunk_id 关联、顶层无标题兜底、表格整表摘要、表格行拆分。**非表格图像类**（思维导图/坐标图/照片/影像）：只留图名/bbox 到 PostgreSQL `raw_documents.content_list`，不进入 chunks 表（见 3.1.1 末设计原则） |
...
| D2 | 术语库构建脚本 | `terms/build_icd10.py`、`scripts/seed_terms.py` | 对 alias 文本做 Qwen3-Embedding-8B Dense 编码 → upsert 到 terms_collection；幂等；集成测试 |
...
**目的**：在线查询链路跑通，输入用户 query，经预处理 → 双路召回 → 单阶段多路 RRF 融合 → 多向量去重 → 精排，输出 Top-K chunks（含 original_content + heading_path 引用信息），具备稳定回退策略。
...
| E4 | 单阶段多路 RRF 融合 + 多向量去重 | `src/rag/retrieval/fusion.py` | Dense（1 路）+ Sparse 各维度（各 1 路）→ 单阶段多路 RRF → 按 source_chunk_id 去重（保留 RRF 最高分记录，original_content 冗余存储保证内容一致）→ Top-M；单元测试 |
...
| 编号 | 任务名称 | 状态 | 完成日期 | 备注 |
|------|---------|------|---------|------|
| A1 | 初始化目录树与最小可运行入口 | [~] | | 已:目录树齐、pyproject.toml 装齐(uv pip install -e .)、.venv 就绪、缓存目录全迁 /data/cache;待:`main.py` / FastAPI app 入口 |
...
| A3 | 配置加载与校验 | [ ] | | |
| A4 | pytest 测试基座 | [~] | | 已:tests/ 目录在、`tests/integration/test_reranker_smoke.py` 已写;待:conftest、真跑过测试 |
| A5 | 公共工具模块 | [ ] | | |
| A6 | Prompt 模板骨架 | [ ] | | |
...
| B2 | PostgreSQL 迁移脚本 | [~] | | 已:`src/db/postgres/migrations/0001_raw_documents.sql`(sources + raw_documents 双表 + GIN);待:其他表迁移、Alembic 接入 |
| B3 | Milvus 连接管理 + docs_collection | [~] | | 已:连接管理通过 `terms_collection.py::_ensure_connection` 间接实现;待:`docs_collection` schema + 建库 |
...
| B5 | PostgreSQL raw_documents 表（MinerU 产物存储） | [~] | | 已:迁移 SQL 与 sources 表合并写在 `0001_raw_documents.sql`;待:ORM 类(`src/db/postgres/models.py` 内 RawDocument) |
...
| 调用点 | Schema | 关键字段 | 安全等级 | 失败处理 |
|-------|--------|---------|---------|---------|
| 3.1.3 `enrichment` | `ChunkEnrichmentOutput` | `title: str`, `summary: str`, `tags: list[str]`, `hypothetical_questions: list[str]` | 低 | 最多尝试 2 次；仍失败则跳过该 chunk 的增强，`title`/`summary`/`tags`/`hypothetical_questions` 留空，chunk 仅以 `original_content` 参与检索（精度降低但不丢数据） |
