...
**Sparse Route 专用处理**
1. 关键词识别 (Keyword Extraction)：由 ② build_query Step 1 调 LLM 做医学 NER（`build_query_step1_ner`，详见 §9.3），从 `chief_complaint + present_illness` 直接提取实体，生成症状维度列表。**不做去停用词或文本清洁**：默认患者主诉表达完整,且 LLM NER 对噪音不敏感,额外清洁仅增加延迟无质量增益。
2. 术语扩展 (Synonym Expansion) + 报告语义信号注入：`sparse_queries` 由两类来源合并组成（每项一次独立 BM25 查询，去重 + 长度 ≥ 2 过滤）：
   - **来源 A（EL 链上 symptom 的别名词袋）**：直接复用节点 ② build_query Step 2 产出的 Entity Linking 结果——以已规范化术语的 `concept_id` 为主键，从 `terms_collection`（见 2.4.6）查出该概念下的全部别名（含口语、缩写、英文），每个症状维度的别名拼成一个词袋字符串。过滤长度 ≤ 1 的过短别名防止泛化匹配。
   - **来源 B（report 语义信号直入）**：`report_findings` 的 `positive_findings` 和 `impressions` 每条作为独立 BM25 词袋（已是医学文献语言，无需 EL 归一化）。这类教材高频鉴别词（如"瞳孔散大"、"右额颞线形骨折"）通过 sparse 路直接召回相关 chunk，不再仅靠 Step 3 dense LLM 单点综合。`abnormal_values` 原始数值与 `negative_findings` 仍不进 query（原规则不变）。
...
- **职责**（每轮循环均完整执行以下四步）:
  ...
  **Step 4. Query 构建/改写（Dense 与 Sparse 分路构建）**
  ...
  - **Sparse Route**：`sparse_queries` 由两类来源合并组成（详见 §3.2.1 Step 2）。来源 A：以 Step 2 产出的 `concept_id` 为单位，每个症状维度取其在 `terms_collection` 中的全部别名拼成一个词袋字符串（过滤长度 ≤ 1 的过短别名）；来源 B：`report_findings` 的 `positive_findings` 和 `impressions` 每条作为独立 BM25 词袋（已是医学文献语言，无需 EL 归一化；如"瞳孔散大"、"右额颞线形骨折"这类教材高频鉴别词直接进 sparse 召回，不再仅靠 Dense LLM 单点综合）。去重 + 长度 ≥ 2 过滤后，每条作为一次独立 BM25 查询，N 条 = N 次查询，RRF 等权融合（每条贡献 1 票）。`abnormal_values` 原始数值与 `negative_findings` 仍不进 query（数字无向量语义；否定词与 BM25/embedding 语义冲突）。
...
| `sparse_queries` | `list[str]` | `build_query` ② | Sparse 路检索 queries：EL 链上 symptom 的别名词袋 + `report_findings` 的 `positive_findings`/`impressions` 每条独立词袋（去重 + 长度 ≥ 2 过滤）；每条一次 BM25，RRF 等权融合（见 §3.2.1 Step 2） |
