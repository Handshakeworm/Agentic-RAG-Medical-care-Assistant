...
| `chart` | caption + markdown 数据表 | text LLM 看 markdown → 医学陈述 |
| `image-flowchart` | caption + mermaid 代码 | text LLM 看 mermaid → 医学陈述 |

source chunk 与 summary chunk 通过 PG `chunks.linked_chunk_id`(summary 反指 source)关联,共享同一 `heading_path_id`。Milvus 命中 summary 后回 PG 拉源 chunk 作为 LLM 消费 payload(含 html/mermaid + 截图路径)。
...
**LLM 选型与成本约束(留给 §3.1.3 / C5 任务实现时定)**:summary 生成默认走 text LLM(看 mineru 提取的 html/markdown/mermaid 文本,而非截图),因为:
- mineru 转录质量高(table html / chart markdown 直接结构化;flowchart 已转 mermaid 包含全部节点+连线)
- text LLM 成本仅为 vision LLM 的 1/10,全 12 本书 summary 生成约 ~$100 vs ~$340

如果将来发现 mermaid OCR 错误率过高,可针对 flowchart 改走 vision fallback。

---

> **关于图像内容理解的设计原则(2026-05-03 修正)**:
> 1. **教科书图配文约定**:图表达的核心信息一般在周围正文里重复,正文召回往往可以兜底,但**对纯数据图(检验值表、诊断流程图)正文无法替代** —— 这是引入 summary chunk 的根本理由
> 2. **不依赖 Vision LLM 看截图**:summary 生成统一走 text LLM 看 mineru 转录(html/markdown/mermaid),避免 Vision LLM 对医学图像的幻觉风险
> 3. **医疗合规**:诊断依据可追溯到权威文本(源 chunk 含 mineru 结构化转录,可校验);summary chunk 是召回辅助层,不作为权威依据出现在最终诊断引用中
>
...
| `child`(子块) | 走完整 enrichment(title + summary + tags + hypothetical_questions) |
...
**规则 2:summary chunk → 源 chunk**(图表 payload 跟随,见 §3.1.2):
...
**去重**:三条规则展开后按 chunk_id 去重(常见 case:summary 命中触发规则 2,父块展开触发规则 3,同一图表被两条路径都拉出来)。
...
| C2 | Chunking(父子分块 + 表格双粒度) | [~] | | 已:**step1 title.level 重建**(已弃案,见 §3.1.1 限制 2 / §3.1.2,改用目录权威清单);**step2 block extractor**(`extract_chunkable_text` 已实现,15 unit PASS);**POC 验证完整切分主流程**(`scripts/poc_chunking_内分泌代谢病学_第4版上册/poc_chunk_book.py`,《内分泌代谢病学第4版上册》 1204 父块 / 3012 子块 / mismatch=0,详见 [METHODOLOGY.md](scripts/poc_chunking_内分泌代谢病学_第4版上册/METHODOLOGY.md));待:**step3 把 POC port 到 production** `chunking.py` 主流程(目录字典+正文匹配+三遍切+严格层级合并+size 驱动子块切+书末截断+参考文献丢弃) / step4 table+chart 双粒度 / step5 12 本书逐本 anchor pattern 适配 + 切块验证 |
