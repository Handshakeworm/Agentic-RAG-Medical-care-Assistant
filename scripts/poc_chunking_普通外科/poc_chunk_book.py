...
====================================================================
**只针对《普通外科》POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L2,12 章 + 77 节 = 89 entries)
  - poc_match_body_titles(Step 2 正文匹配)

本书规则:
  - **字典 L1-L2**(无篇,跟消化字典层级类似但消化含 一、)
  - **子标题层级:章 → 节(已在字典)→ 一、 → (一) → 1.,无【】**
  - **Pass 起步**:跟胸心外科同 一、+(一)+1.(字典已含节)
  - **BODY_END = 中英文名词对照索引**(pg 697 起索引页丢弃)
  - **absorb_levels = (1, 2)**:本书字典只到 L2,L1/L2 短 section 跨吸收
...
def _is_dunhao_subheading(text, *, block_type=None):
...
def _is_paren_subheading(text, *, block_type=None):
    if block_type == "list": return False
    s = text.strip()
...
def main():
    ...
    print(f"PARENT_REFINE_THRESHOLD: {PARENT_REFINE_THRESHOLD} (Pass 2 (一))")
    print(f"PARENT_PASS3_THRESHOLD: {PARENT_PASS3_THRESHOLD} (Pass 3 1.)")
