...
====================================================================
**只针对《普通外科》POC** —— 字典构建依赖 poc_build_toc_dict.py。

输入:权威字典 89 条(12 章 + 77 节),L1-L2 用于 title 匹配
正文范围:page_idx > max(toc_pages),即 pg 20 起
"""
...
# 兼容 `第N章` 单独 title + 章名紧跟另一 title 的情况(本书无篇,字符类只 `章` 也可,保留 `[篇章]` 无副作用)
...
def main() -> None:
    ...
    print(f"=== Part aliases: {len(part_alias)} (本书无篇,应为 0)===")
    ...
    print(f"=== Missing (L1-L2): {len(missing)} ===")
