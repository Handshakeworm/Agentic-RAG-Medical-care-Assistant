...
本书规则(无篇,字典 L1-L2):
  L1 = 第N章(12 个)
  L2 = 第N节(目录最细颗粒度,~77 个)
  字典深度到 L2 节 — 跟消化(无篇)同结构,但无 一、 在字典

正文起点:pg 20(第一章 外科总论)
末尾截断:pg 697 起 `中英文名词对照索引`
"""
...
CONTENT_LIST_V2 = (
    ...
    "普通外科/hybrid_auto/"
    "普通外科_content_list_v2.json"
)

# 2 类 anchor — 本书字典 L1=章 / L2=节(无篇)
...
# 兼容单个 `…` 尾页码(胸心外科 §3.2 升级):后置 \d+\s*$ 已限定行末数字结尾
...
def _detect_toc_pages(data: list) -> list[int]:
    """本书 pg 18 (title='目录') + pg 19 (page_header='目录') 双 seed 已覆盖完整 TOC。
    只填充 seeds 间间隙,不向外延伸(胸心外科 §3.1 选项)。
    """
