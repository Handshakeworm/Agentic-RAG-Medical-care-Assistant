...
====================================================================
**只针对《现代泌尿外科学》POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L4,14 篇 + 93 章 + 504 节 + 1928 一、 = 2539 entries)
  - poc_match_body_titles(Step 2 正文匹配)

本书规则:
  - **字典 L1-L4**(11 本最大字典 2539 entries,含 一、 子标题)
  - **子标题层级:篇 → 章 → 节 → 一、(已在字典)→ (一) → 1.,无【】**
  - **Pass 起步**:字典已含 L4 → Pass 1 = (一)
  - **BODY_END = 索引**(type=title,pg 1678)
  - **absorb_levels = (1, 2, 3)**:L4 是叶子不吸收
...
PARENT_REFINE_THRESHOLD = 1_000_000_000  # Pass 2 1. 关闭
...
def _find_body_end(flat: list[dict]) -> int:
    """本书 BODY_END `索引` 是 type=title(pg 1678),骨科是 paragraph,这里通用兼容两种。"""
...
def _real_start_positions(flat, result):
