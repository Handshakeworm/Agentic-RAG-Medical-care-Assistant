...
====================================================================
**只针对《神经外科学》POC** —— 字典构建依赖 poc_build_toc_dict.py(本书规则)。

输入:权威字典(L1-L3 共 195 条:7 篇 + 33 章 + 155 节)
正文范围:page_idx > max(toc_pages),即 pg 8 起

跟前几本的差异:
  - **字典只到 L3**(同诊断学,无 L4)
  - **篇 anchor 干净**:7 篇全是单 title block 完整(无诊断学/内科学的"第N篇" 拆 bug)
    → CHAP_MERGED 保留代码但可能 0 触发
  - **正文 page_header**:章首页之外的页 page_header 是书名"神经外科学"或章名
    → PAGE_HEADER_FB 可能救少数 mineru 漏标 type=title 的章
  - **OCR 字间空格**:第十七/十八章章名"动脉粥样硬化性 脑血管病" 中间多空格
    → strict_key 去空白后能匹配,FUZZY 可能不需要
