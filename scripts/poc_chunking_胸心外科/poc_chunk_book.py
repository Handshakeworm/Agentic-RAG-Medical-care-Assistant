...
====================================================================
**只针对《胸心外科》POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L3,2 篇 + 15 章 + 74 节 = 91 entries)
  - poc_match_body_titles(Step 2 正文匹配)

本书规则:
  - **字典 L1-L3**(2 篇 + 15 章 + 74 节,跟心血管同结构)
  - **子标题层级:篇 → 章 → 节(已在字典)→ 一、 → (一) → 1.,无【】**
  - **Pass 起步**:跟心血管同 一、+(一)+1.(字典已含节)
  - **BODY_END = 中英文名词对照索引**(pg 513 起索引页 + pg 524 广告页丢弃)
  - **absorb_levels = (1, 2, 3)**:篇/章/节都参与短 section 跨吸收
...
# 字典含节,Pass 切字典外的更细层:一、 → (一) → 1.

# Pass 1 anchor:一、二、(中文+顿号)
...
# ─────────────────────────────────────────────────────────────────────
# 阈值(baseline 阶段:Pass 关闭,先看纯字典粒度分布)
# ─────────────────────────────────────────────────────────────────────
