...
====================================================================
**只针对《协和呼吸病学》(第二版)POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L2,无 L3/L4)
  - poc_match_body_titles(Step 2 正文匹配)

跟前 5 本的差异:
  - **字典最浅 L1-L2**(16 篇 + 163 章,无节无 L4),Pass 1 必开(章平均 14000 字)
  - **Pass 1 anchor 用 第N节**(本书首次,前 5 本用 一、 / (一))
  - **Pass 2 anchor 用 【】**(同内分泌/内科学/诊断学)
  - **BODY_END = 索引**(pg 2651 起是字母索引页,丢弃)
  - **absorb_levels = (1, 2)**:无 L3,只 L1/L2 短 section 跨吸收
  - 节末"参考文献"丢弃(本书有,跑得到)

baseline 先 PARENT_SPLIT_THRESHOLD = 10**9(Pass off),看纯目录粒度分布
再决定阈值(SOP §7.5)。
"""
...
# 书末 BODY_END:首个 type=title `索引`(pg 2651)— 之后是字母索引页
BODY_END_MARKERS: tuple[str, ...] = ("索引",)
...
# ─────────────────────────────────────────────────────────────────────
PARENT_SPLIT_THRESHOLD = 6000          # user 拍板 2026-05-05:Pass 1 第N节 开,阈值 6000
PARENT_REFINE_THRESHOLD = 6000         # user 拍板 2026-05-05:Pass 2 【】开,救节内 / 无节大父块
PARENT_PASS3_THRESHOLD = 6000          # user 拍板 2026-05-05:Pass 3 (一) 开,救【】内 / 整章无节无【】大父块
PARENT_PASS4_THRESHOLD = 6000          # user 拍板 2026-05-05:Pass 4 1. 开,救【】内部纯列表 / (一) 内连贯论述
...
def _is_jie_subheading(text: str, *, block_type: str | None = None) -> bool:
    """Pass 1 切分点:第N节(本书 anchor)"""
    if block_type == "list":
        return False
    s = text.strip()
    if len(s) < 4:
...
def _is_bracket_subheading(text: str, *, block_type: str | None = None) -> bool:
    ...
    if RE_TABLE_TITLE.match(s) or RE_FIG_TITLE.match(s):
        return False
    return bool(RE_BRACKET.match(s))
...
def chunk_book() -> dict:
    ...
    for i in range(len(section_splits)):
        ...
        # 本书字典只 L1-L2,absorb_levels=(1,2)(L3 节由 Pass 1 切,不入字典)
        absorb_levels = (1, 2)
...
def main():
    ...
    print("=" * 70)
    print("切分流程结果(协和呼吸病学,baseline 纯目录粒度,Pass off)")
    print("=" * 70)
