...
====================================================================
**本规则只针对《协和呼吸病学》(第二版)实测有效**;通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书特定笔记见
[`BOOK_NOTES.md`](BOOK_NOTES.md)。

本书规则:
  L1 = 第N篇(16 个,第一..第十六 连续)
  L2 = 第N章(163 个,跨篇连续编号)
  **字典只到 L2 章** — 目录不列节。前 5 本最浅是神经外科 L1-L3,本书更浅。

跟前 5 本的差异:
  - **字典最浅**(L1-L2,无节无 L4),Pass 1 必开
  - **TOC 6 页**(pg 12..17),双栏多行
  - **mineru type 漂移**:篇有时 title 有时 paragraph(下半部分篇名都是 paragraph),
    神经内科学已暴露过,沿用 type=title+paragraph+list 三种来源
  - **正文 节 anchor 强**(336 处 type=title `第N节`),Pass 1 直接用节,不靠 一、 / (一)
"""
...
CONTENT_LIST_V2 = (
    ...
    "协和呼吸病学(第二版)  高清版/hybrid_auto/"
    "协和呼吸病学(第二版)  高清版_content_list_v2.json"
)

# 2 类 anchor — 本书字典只到 L2 章
PATTERNS: list[tuple[int, re.Pattern]] = [
    (1, re.compile(r"^第\s*\S{1,4}\s*篇(?=\s|$)")),
    (2, re.compile(r"^第\s*\S{1,4}\s*章(?=\s|$)")),
]

# 跨条目粘连二次拆分:同神经内科学,只切篇/章/节(节本书不入字典但保留以防漏切)
...
def build_toc_dict() -> dict:
    ...
    # 同 page 相邻行合并:本书字典只到章,无 L3/L4 anchor 切碎风险,关闭多行合并
    # (前 5 本是为了救 L3/L4 切碎才开的同 page 合并)
    ...
    for pg, line in expanded:
        s = line.strip()
        # 归一化后检查 BLACKLIST,救尾页码污染的非内容行(如 "索引 (2634)" / "彩图插页 (2655)")
        norm_s = _normalize(s)
