...
====================================================================
**只针对《协和呼吸病学》(第二版)POC** —— 字典构建依赖 poc_build_toc_dict.py。

输入:权威字典 179 条(16 篇 + 163 章),L1-L2 用于 title 匹配
正文范围:page_idx > max(toc_pages),即 pg 18 起

跟前 5 本的差异:
  - **字典最浅**(L1-L2 only,无节)— L3 节交给 Step 3 Pass 1 切
  - **TITLE_ALONE_RE 兼容篇/章**(篇也常被切成 `第一篇` + `呼吸内科学基础理论` 两 title)
  - **PATCH_BODY_RAW_FIX**:救 mineru body OCR 错字(鼾→肝),
    不靠降 FUZZY 阈值(短标题 1 字差 ratio=0.8,降阈值会引入跨章误配)

5 类 candidate action(继承 SOP):
...
# 兼容篇/章独立 title(`第一篇` 单独 title + 篇名/章名 紧跟另一 title 的情况)
...
# 协和呼吸 pg 1801 `第二章 肝症` mineru OCR 把"鼾"识成"肝",字典是"鼾症";
# FUZZY ratio=0.8 < 0.85 阈值,降阈值会引入跨章误配 → 走精确 patch
...
def _build_part_alias(entries: list) -> dict[str, str]:
    """L1 篇名 → 完整 `第N篇 名称` 反查(救 PART_REBUILT 分裂封面)。"""
    alias: dict[str, str] = {}
...
def _collect_candidates(
    ...
    for pg_idx in range(body_start, len(data)):
        ...
        for i, b in enumerate(blocks):
            ...
            if btype == "title":
                ...
                # 篇/章 alone — 找下一 title 拼起来(`第一篇` + `呼吸内科学基础理论`)
                ...
                key = strict_key(raw)
                # 篇首字 alias(`呼吸内科学基础理论` 单独 title 时回填 `第一篇 ...`)
    ...
    # FUZZY:救 OCR 错字(章名)/ 字间空格 等;只跑 L2 章字典
    chapter_dict_keys = {
...
def main() -> None:
    ...
    dict_l12_entries = [e for e in entries if e[0] <= 2]

    print(f"=== Dict for matching: {len(lookup)} unique keys (L1-L2) ===")
    print(f"=== Total entries: {len(entries)} ===")
    print(f"=== Body scan range: page_idx {body_start}..{len(data) - 1} ===")
    print(f"=== Part aliases: {len(part_alias)} entries ===")
    for k, v in list(part_alias.items())[:5]:
        print(f"    {k!r} → {v!r}")
    if len(part_alias) > 5:
        print(f"    ... ({len(part_alias)-5} more)")
    print()
    ...
    missing = [
        (level, title, path, pg)
        for level, title, path, pg in dict_l12_entries
    ...
    if len(matched) > 20:
        print(f"  ... ({len(matched) - 20} more matched lines)")

    print(f"\n=== Missing (in L1-L2 dict but not found in body): {len(missing)} ===")
    ...
    print(f"  (预期含 第N节 / 【...】 / (一) / 1. 等更细子节,Step 3 Pass 1/2/3 切到这粒度)\n")

    print(f"\n=== Summary ===")
    print(f"  Dict L1-L2:                  {len(lookup)}")
    print(f"  Body candidates (after pp):  {len(candidates)}")
    print(f"  Matched:                     {len(matched)}")
    print(f"    coverage of L1-L2 dict:    {100*(len(lookup)-len(missing))/max(len(lookup),1):.1f}%")
    print(f"  Missing in body (L1-L2):     {len(missing)}")
