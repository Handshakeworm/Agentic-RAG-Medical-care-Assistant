...
PARENT_SPLIT_THRESHOLD = 6000        # > 6000 触发 Pass 1 一、
PARENT_REFINE_THRESHOLD = 6000       # Pass 2 (一) 开,>6000 进一步切(神经内科学)
...
def _real_start_positions(flat: list[dict], result: dict) -> dict[int, tuple[int, str, str]]:
    """每个 (level, full_path) 选 1 个最佳位置作为 section 起点。

    **关键修正(2026-05-05)**:strict_key 冲突处理
      神经内科学:`第一节概述` 一个 strict_key 命中 17 个不同章的 entry。
      若按 (level, title) 分组,17 处 sections 被合并成 1 个,16 处位置丢失,
      实际正文里这 16 章的"第一节"内容会吸收到上一节,造成父块严重错位。
      修正:cands 多于 1 个时用当前 stack 消歧;groups 按完整 path 分组。

    返回 dict[pos] = (level, title, full_path) — 增加 full_path 让下游能区分同名 sections。
    """
    ...
    for pg_idx, blk_idx, raw, key, action in candidates:
        ...
        if not cands:
            continue
        # 用 stack 消歧:候选 entry 的 parent_path 和当前 stack 匹配的优先
        if len(cands) == 1:
    ...
    # 按完整 path 分组(而不是 (level, title)),让 17 处"第一节 概述"分到 17 个 group
    ...
    for _key, recs in groups.items():
        ...
        if strong:
            ...
            # 退化:AS_IS 优先 PAGE_HEADER_FB(神经外科学的教训)
...
def chunk_book() -> dict:
    ...
    # 前言/序章丢弃:first section 起点之前的 block 都不是医学正文
    # (神经内科学:pg 21-22 编者前言"刘鸣 谢鹏",2892 字)
    ...
    for i in range(len(section_splits)):
        ...
        # L1/L2/L3 短 section → 跨 section 吸收;L4 是最深层永远保留
        # 神经内科学:字典含 L4(487 个),L3 标题后立即跟 L4 时区间只剩 title 自己
        # (前 4 本 L3 是叶子或 L3 后接 paragraph,不空壳,所以代码写死 (1,2) 没暴露)
        absorb_levels = (1, 2, 3)
...
def main():
    ...
    print("=" * 70)
    print("切分流程结果(神经内科学,baseline 纯目录粒度)")
    print("=" * 70)
