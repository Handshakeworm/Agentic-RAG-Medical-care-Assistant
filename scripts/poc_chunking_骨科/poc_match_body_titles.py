...
====================================================================
**只针对《骨科》POC** —— 字典构建依赖 poc_build_toc_dict.py。

输入:权威字典 1293 条(6 篇 + 51 章 + 253 节 + 983 一、),L1-L4 用于 title 匹配
正文范围:page_idx > max(toc_pages),即 pg 44 起
"""
...
PATCH_BODY_RAW_FIX: list[tuple[str, str]] = [
    # mineru OCR 错字:踇外翻(hallux valgus)在字典识为"蹇",正文 pg1197 识为"蹮",统一到字典写法
    ("第一节 蹮外翻", "第一节 蹇外翻"),
    # L4 子标题末尾带英文术语括号 — 字典侧没有,正文需去掉以对齐 strict_key
...
def main() -> None:
    ...
    dict_l14_entries = entries  # 全部级别都看 missing
    ...
    missing = [
        (level, title, path, pg)
        for level, title, path, pg in dict_l14_entries
