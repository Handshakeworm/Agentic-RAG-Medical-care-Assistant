...
====================================================================
**只针对《骨科》POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L4,6 篇 + 51 章 + 253 节 + 983 一、 = 1293 entries)
  - poc_match_body_titles(Step 2 正文匹配)

本书规则:
  - **字典 L1-L4**(深字典,含 一、 子标题 — TOC 列出)
  - **子标题层级:篇 → 章 → 节 → 一、(已在字典)→ (一) → 1.,有【】(383 个)**
  - **Pass 起步**:字典已含 L4 → Pass 1 = (一)(跟消化同)
  - **BODY_END = `索引`**(paragraph 形态,本书 pg 1444 末尾混乱区)
  - **absorb_levels = (1, 2, 3)**:L4 是叶子不吸收(跟内分泌/神内同)
...
# 字典含 一、,Pass 切字典外的更细层:(一) → 1.;另有 383 个 【】 待评估
RE_PAREN_CN = re.compile(r"^[（(][一二三四五六七八九十百]+[)）]")
RE_NUMDOT = re.compile(r"^\d+\s*[\.、]\s*\S")
RE_BRACKET = re.compile(r"^【[^】]+】")

# 书末 BODY_END:paragraph 形态 `索引`(pg 1444,本书末尾混乱)
...
PARENT_PASS3_THRESHOLD = 1_000_000_000   # Pass 3 未用
...
def _find_body_end(flat: list[dict]) -> int:
    """本书 BODY_END 在 paragraph 形态(pg 1444 末尾混乱区),不只看 title。"""
...
def _is_paren_or_bracket_subheading(text, *, block_type=None):
    """Pass 1 切分点:(一) 或 【...】 — 本书子标题混用两种结构(感染章用【】,其他多 (一))"""
    if block_type == "list": return False
    s = text.strip()
    if len(s) < 4: return False
    if RE_TABLE_TITLE.match(s) or RE_FIG_TITLE.match(s): return False
    return bool(RE_PAREN_CN.match(s) or RE_BRACKET.match(s))


def _is_numdot_subheading(text, *, block_type=None):
    if block_type == "list": return False
    s = text.strip()
    if len(s) < 4: return False
    if RE_TABLE_TITLE.match(s) or RE_FIG_TITLE.match(s): return False
...
def _find_ref_idx(section_blocks):
...
def _split_big_parent(section_blocks, threshold, ref_idx):
    ...
    pass1 = _refine([(0, LEVEL_SECTION)], _is_paren_or_bracket_subheading, LEVEL_PAREN, threshold)
...
def chunk_book() -> dict:
    ...
    for i in range(len(section_splits)):
        ...
        # 字典 L1-L4,absorb_levels=(1,2,3),L4 是叶子不吸收
        absorb_levels = (1, 2, 3)
...
def main():
    ...
    print(f"PARENT_PASS3_THRESHOLD: {PARENT_PASS3_THRESHOLD}")
    ...
    parent_buckets = [(0, 499), (500, 1999), (2000, 4999), (5000, 9999),
                      (10000, 19999), (20000, 79999)]
    child_buckets = [(0, 199), (200, 499), (500, 999), (1000, 1999),
                     (2000, 4999), (5000, 9999)]
