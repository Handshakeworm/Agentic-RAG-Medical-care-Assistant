...
def main() -> None:
    ...
    # 字典 strict_key 重复时(`第一节 概述` 多章共有),需要按 full_path 去重 而非 strict_key
    chosen: dict[str, tuple] = {}  # key: full_path
    for m in matched:
        full_path = m[5]
        if m[6] not in strong_actions:
            continue
        if full_path not in chosen:
            chosen[full_path] = m
    ...
    for level, title, path, _ in entries:
        if path in chosen:
            m = chosen[path]
            ordered.append((path, m[0], m[1], level, title, strict_key(title)))
    ...
    bad_nest = []
    current_pian = None
    for _path, pg, blk, level, title, _k in ordered:
        if level == 1:
            current_pian = title
        elif level == 2:
    ...
    if declared:
        ...
        # strict_key 重复的 entries (`第一节 概述` 类) declared 无法 disambig,跳过
        ambiguous_keys = {k for k, v in lookup.items() if len(v) > 1}
        offsets = []
        skipped_ambiguous = 0
        for _path, pg, blk, level, title, k in ordered:
        ...
        if skipped_ambiguous:
            print(f"  (跳过 {skipped_ambiguous} 条 strict_key 重复的 entries — declared 无法 disambig)")
        # 看 offset 分布
