...
本书规则:
  ...
  L2 = 第N章(45 个)
  L3 = 第N节(目录最细颗粒度)
  字典深度到 L3 节 — 跟内分泌/诊断学一样的深字典

正文起点:pg 20(第一篇/第一章 同页)
末尾截断:pg 717 `参考文献` / pg 771 `中英文名词对照索引` / pg 781 广告页(BLACKLIST)
"""
...
CONTENT_LIST_V2 = (
    ...
    "心血管内科学 第3版/hybrid_auto/"
    "心血管内科学 第3版_content_list_v2.json"
)

# 3 类 anchor — 本书字典到 L3 节
...
BLACKLIST = {
    "上册", "下册", "目录",
    "心血管内科学", "心血管内科学（第3版）", "心血管内科学(第3版)",
    "索引", "中英文名词对照索引",
    "公众号登录 >>", "网站登录 >>", "进入中华临床影像库首页", "注册或登录",
    "临床影像库", "登录中华临床影像库步骤",
}

PATCH_REPLACE_TITLE: dict[str, str] = {}
PATCH_LINE_PREPROCESS: list[tuple[str, str]] = []
# 第八篇 mineru 页眉滞后 1 页(实际篇首在 pg 528,page_header 落在 pg 529),
# 注入到 pg 528 b=-2 让其排在第三十三章 (pg 528 b=0) 前面 + dedup 时删 PAGE_HEADER_FB
PATCH_INJECT_CANDIDATES: list[tuple[int, int, str]] = [
    (528, -2, "第八篇 心血管疾病危险因素管理"),
]
...
def build_toc_dict() -> dict:
    ...
    # 规则:同页 + 前行无 TAIL_PAGE 尾 + 后行不是新 anchor → 把后行拼到前行
