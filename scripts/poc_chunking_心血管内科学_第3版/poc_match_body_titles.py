...
====================================================================
**只针对《心血管内科学》(第3版)POC** —— 字典构建依赖 poc_build_toc_dict.py。

输入:权威字典 239 条(11 篇 + 45 章 + 183 节),L1-L3 用于 title 匹配
正文范围:page_idx > max(toc_pages),即 pg 26 起
"""
...
def _collect_candidates(
    ...
    for pg_idx in range(body_start, len(data)):
        ...
        for i, b in enumerate(blocks):
            ...
            if btype == "title":
                ...
                if key not in lookup:
                    continue
                # blk_idx=-1 让 PAGE_HEADER_FB 排在该页 b0 之前,stack 先更新到章/篇,
                # 后续重复 strict_key (第一节 概述等) 才能 disambig 到正确章
    ...
    # 同 key 已有 HARDCODE → 删 PAGE_HEADER_FB(避免重复 strong 位置)
