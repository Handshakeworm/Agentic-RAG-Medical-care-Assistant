...
====================================================================
**只针对《心血管内科学》(第3版)POC**(规则书本特化,通用方法论见
[`scripts/METHODOLOGY.md`](../METHODOLOGY.md),本书笔记见 BOOK_NOTES.md)。

依赖:
  - poc_build_toc_dict(Step 1 字典 L1-L3,11 篇 + 45 章 + 183 节)
  - poc_match_body_titles(Step 2 正文匹配)

本书规则:
  - **字典 L1-L3**(11 篇 + 45 章 + 183 节)
  - **Pass 1 anchor 用 第N节**(字典已含节,Pass 1 主要兜底字典外节)
  - **Pass 2-4 同协和呼吸 SOP**(【】/(一)/1.)
  - **BODY_END = 中英文名词对照索引**(pg 771 起索引/广告页丢弃)
  - **absorb_levels = (1, 2, 3)**:L3 节也参与短 section 跨吸收
  - 节末"参考文献"按 RE_REF_MARKER 丢弃
"""
...
# 书末 BODY_END:首个 type=title `中英文名词对照索引`(pg 771)— 之后是字母索引页 + 广告页
...
# ─────────────────────────────────────────────────────────────────────
# 阈值(baseline 阶段:Pass off,先看纯目录粒度分布)
# ─────────────────────────────────────────────────────────────────────
PARENT_SPLIT_THRESHOLD = 6000          # Pass 1 一、 阈值
PARENT_REFINE_THRESHOLD = 6000         # Pass 2 (一) 阈值
PARENT_PASS3_THRESHOLD = 6000          # Pass 3 1. 阈值
PARENT_MERGE_TINY_THRESHOLD = 500
CHILD_SPLIT_THRESHOLD = 1200
CHILD_TARGET_SIZE = 600
CHILD_MIN_SIZE = 200

# 跨 section 吸收:本书字典无 L3/L4,只 L1/L2 参与
...
def _is_dunhao_subheading(text: str, *, block_type: str | None = None) -> bool:
    """Pass 1 切分点:一、二、(中文+顿号)"""
    if block_type == "list":
        return False
    s = text.strip()
    if len(s) < 3:
...
LEVEL_SECTION = 0
LEVEL_DUNHAO = 1    # 一、(本书 Pass 1)
LEVEL_PAREN = 2     # (一)(本书 Pass 2)
LEVEL_NUMDOT = 3    # 1.(本书 Pass 3)
...
def _split_big_parent(section_blocks: list[dict], threshold: int,
    ...
    pass1 = _refine([(0, LEVEL_SECTION)], _is_dunhao_subheading, LEVEL_DUNHAO, threshold)
    pass2 = _refine(pass1, _is_paren_subheading, LEVEL_PAREN, PARENT_REFINE_THRESHOLD)
    pass3 = _refine(pass2, _is_numdot_subheading, LEVEL_NUMDOT, PARENT_PASS3_THRESHOLD)
    return pass3
...
def chunk_book() -> dict:
    ...
    for i in range(len(section_splits)):
        ...
        # 本书字典 L1-L3,absorb_levels=(1,2,3)(L3 节也参与短 section 跨吸收)
        absorb_levels = (1, 2, 3)
...
def main():
    ...
    print("=" * 70)
    print("切分流程结果(心血管内科学 第3版)")
    print("=" * 70)
    print(f"PARENT_SPLIT_THRESHOLD: {PARENT_SPLIT_THRESHOLD}")
    print(f"PARENT_REFINE_THRESHOLD: {PARENT_REFINE_THRESHOLD}")
    ...
    threshold = PARENT_SPLIT_THRESHOLD
    big_parents = sorted([p for p in parents if p["len"] > threshold],
                         key=lambda p: -p["len"])
    print(f"\n仍 > {threshold} 字父块: {len(big_parents)} 个(showing top 20)")
