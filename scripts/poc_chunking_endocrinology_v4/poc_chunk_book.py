...
依赖:
  - poc_build_toc_dict_endocrinology_v4(Step 1 目录字典)
  - poc_match_body_titles_endocrinology_v4(Step 2 正文匹配)
...
from poc_match_body_titles_endocrinology_v4 import (
    _build_part_alias,
    _collect_candidates,
)
...
# 阈值(用户拍板,2026-05-03)
# ─────────────────────────────────────────────────────────────────────
PARENT_SPLIT_THRESHOLD = 4000       # 父块 > 4000 字 (~2877 token) → Pass 1+2: 【】+(一)(二)
                                    # 实测 Qwen tokenizer 1 token ≈ 1.39 字符,目标父块 ~3000 token
PARENT_PASS3_THRESHOLD = 5000       # 父块 > 4999 字 (~3597 token) → Pass 3: 加 1./2. 细化
                                    # 4000-4999 接受不再切(避免把医学小节切碎)
...
def _real_start_positions(flat: list[dict], result: dict) -> dict[int, tuple[int, str]]:
    ...
    for key, recs in groups.items():
        ...
        chosen = strong[-1] if strong else len(recs) - 1
